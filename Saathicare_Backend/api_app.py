...
def interactive_physician_chatbot():
    ...
    if tag == 'symptom':
        instance = {"prompt": question,
                ...
                "top_p": 1.0,
                "top_k": 10}
        prediction = predict_vertex_ai(ENDPOINT_ID, PROJECT_ID, instance)[0].replace('*', '').split('Symptom:')[-1].split('\n')[0].strip()
    elif tag == 'lifestyle':
        instance = {"prompt": question,
                "max_tokens": 512,
                "temperature": 1.0,
                "top_p": 1.0,
                "top_k": 10}
        prediction = predict_vertex_ai(ENDPOINT_ID, PROJECT_ID, instance)[0].replace('*', '').split('Lifestyle:')[-1].split('\n')[0].strip()
    elif tag == 'genetic':
        instance = {"prompt": question,
                "max_tokens": 512,
                "temperature": 1.0,
                "top_p": 1.0,
                "top_k": 10}
        prediction = predict_vertex_ai(ENDPOINT_ID, PROJECT_ID, instance)[0].replace('*', '').split('Genetic:')[-1].split('\n')[0].strip()
    elif tag == 'report':
        instance = {"prompt": question,
                "max_tokens": 512,
                "temperature": 1.0,
                "top_p": 1.0,
                "top_k": 10}
        prediction = predict_vertex_ai(ENDPOINT_ID, PROJECT_ID, instance)[0].split('END OF RESPONSE\nOutput:')[-1].strip().replace('*', '').replace('END OF RESPONSE', '').replace('"', '')
    else:
